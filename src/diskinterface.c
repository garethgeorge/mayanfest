...
Disk *disk_create(Size chunk_count, Size chunk_size) {
...
Size disk_get_size_bytes(Disk *disk) {
	return disk->chunk_count * disk->chunk_size;
}

int disk_read_chunk(Disk *disk, Size block_idx, Byte *outbuf) {
...
int disk_write_chunk(Disk *disk, Size block_idx, Byte *data) {
	if (block_idx >= disk->chunk_size)
		return -1;
